...
int scan(const char *pathstart){
	...
	int scandir_len = 0;
	if (!(scandir = opendir(scanning_path))) panic(-1, _("Error @ %s:%d:scan(...) - Failed to open %s"), __FILE__, __LINE__, scanning_path);
	while ((scanfile = readdir(scandir))){
		...
		if (!(temp_path = malloc(scandir_len + 1))) panic(-2, _("Error @ %s:%d:scan(...) - Failed to malloc(...)"), __FILE__, __LINE__);
		strcpy(temp_path, scanning_path);
		strcat(temp_path, "/");
		strcat(temp_path, scanfile->d_name);
		temp_path[strlen(temp_path)] = '\0';
		if (stat(temp_path, &scannedfile_stat) < 0) panic(-3, _("Error @ %s:%d:scan(...) - Failed to get stat on %s"), __FILE__, __LINE__, temp_path);
		...
		if (S_ISREG(scannedfile_stat.st_mode)){
			...
			if (token){
				...
				if (!(yyin = fopen(temp_path, "r"))) panic(-4, _("Error @ %s:%d:scan(...) - Failed to open %s for scanning"), __FILE__, __LINE__, temp_path);
				else{
					yylex();
					fclose(yyin);
				}
	...
	if (closedir(scandir) < 0) panic(-5, _("Error @ %s:%d:scan(...) - Failed to close %s"),  __FILE__, __LINE__, scanning_path);
	return 1;
}
void print_permissions(void){
	fprintf(stdout, _("=> END SCANNING...\n\n"));
	if (permissions_bmp > 0){
		fprintf(stdout, _("=> RESULT(S):\n"));
