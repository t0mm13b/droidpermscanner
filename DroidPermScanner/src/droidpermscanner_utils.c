...
void show_help(const char *self_binary){
	fprintf(stdout, "%s - Android's Java source code scanner and deduces appropriate AndroidManifest.xml's permissions required\n\n", PACKAGE_STRING);
	fprintf(stdout, "Usage: %s [-s path] [-h] [-v]\n\n", self_binary);
	fprintf(stdout, "-s, --scan   \tscan specified directory source\n");
	fprintf(stdout, "-v, --version\tprint version then exit\n");
...
void show_version(void){
	fprintf(stdout, "%s.\n", PACKAGE_STRING);
	fprintf(stdout, "Copyright (C) 2012 Tom Brennan\n");
